...
void app_main(void)
{
    ...
    uint64_t delay = esp_timer_get_time();
    ...
    while (1)
    {
        ...
        //     continue;
        // }
        // GPIO_SEL_6 | GPIO_SEL_13 | GPIO_SEL_5;

        if (gpio_get_level(GPIO_NUM_5) == 0 && esp_timer_get_time() - delay > 8000)
        {
            delay = esp_timer_get_time();
            ESP_LOGI("BUTTON ONE (1)", "Click");
        ...
        if (gpio_get_level(GPIO_NUM_6) == 0 && esp_timer_get_time() - delay > 8000)
        {
            delay = esp_timer_get_time();
            ESP_LOGI("BUTTON TWO (2)", "Click");
        ...
        if (gpio_get_level(GPIO_NUM_13) == 0 && esp_timer_get_time() - delay > 8000)
        {
            delay = esp_timer_get_time();
            ESP_LOGI("BUTTON THREE (3)", "Click");
            ESP_LOGI(TAG, "[ * ] [Vol-] touch tap event");
            player_volume -= 3;
            if (player_volume < 0)
            {
